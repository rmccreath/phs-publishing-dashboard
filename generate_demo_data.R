...
generate_rich_dashboards <- function(n = 30) {
  ...
  tibble::tibble(
    ...
    # Dates
    deployment_date = as.POSIXct(Sys.Date() - sample(1:730, n, replace = TRUE)),
    last_updated = as.POSIXct(Sys.Date() - sample(1:60, n, replace = TRUE)),
    ...
    last_compliance_check = as.POSIXct(Sys.Date() - sample(1:14, n, replace = TRUE)),
    ...
    # Timestamps
    created_at = deployment_date - sample(7:60, n, replace = TRUE),
...
generate_rich_approvals <- function(dashboards, n = 15) {
  ...
  tibble::tibble(
    ...
    submitted_at = as.POSIXct(Sys.Date() - sample(1:60, nrow(eligible_dashboards), replace = TRUE)),
    ...
    reviewed_at = as.POSIXct(ifelse(
      status != "pending",
      submitted_at + sample(3:14, nrow(eligible_dashboards), replace = TRUE),
      NA
    ), origin = "1970-01-01"),
    ...
    governance_signoff_at = as.POSIXct(ifelse(governance_signoff, reviewed_at + sample(1:3, nrow(eligible_dashboards), replace = TRUE), NA), origin = "1970-01-01"),
    ...
    technical_signoff_at = as.POSIXct(ifelse(technical_signoff, reviewed_at + sample(1:3, nrow(eligible_dashboards), replace = TRUE), NA), origin = "1970-01-01"),
    ...
    security_signoff_at = as.POSIXct(ifelse(security_signoff, reviewed_at + sample(1:5, nrow(eligible_dashboards), replace = TRUE), NA), origin = "1970-01-01"),
